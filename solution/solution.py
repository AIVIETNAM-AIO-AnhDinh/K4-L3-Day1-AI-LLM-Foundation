...
Hướng dẫn:
    ...
    4. Lab dùng Google Gemini qua endpoint TƯƠNG THÍCH OpenAI, nên code vẫn
       viết bằng thư viện `openai` — chỉ khác api_key và base_url.
    5. Import OpenAI BÊN TRONG hàm (xem gợi ý) — nếu import ở đầu file,
       các bài test mock sẽ không hoạt động.
    6. Kiểm tra tiến độ:  pytest tests/test_part1.py -v  (từng phần)
...
PRICING_PER_1K_TOKENS = {
    # Google Gemini — giá bậc trả phí, quy từ USD/1 triệu token
    "gemini-3.5-flash": {"input": 0.0015, "output": 0.009},
    "gemini-2.5-flash": {"input": 0.0003, "output": 0.0025},
    # OpenAI — giữ lại để so sánh giá giữa hai nhà cung cấp
    "gpt-4o": {"input": 0.0025, "output": 0.010},
    "gpt-4o-mini": {"input": 0.00015, "output": 0.0006},
}

# Endpoint tương thích OpenAI của Gemini — nhờ nó ta dùng được thư viện
# `openai` quen thuộc mà vẫn gọi tới model của Google.
GEMINI_BASE_URL = os.getenv(
    "OPENAI_BASE_URL", "https://generativelanguage.googleapis.com/v1beta/openai/"
)

# Tên model có thể đổi qua .env — ví dụ khi dùng NVIDIA NIM miễn phí
# (xem LAB_GUIDE.md, Phụ lục B). Không đặt gì trong .env thì mặc định Gemini.
# Tên biến vẫn giữ tiền tố OPENAI_ vì bộ test tham chiếu tới nó.
OPENAI_MODEL = os.getenv("LAB_MODEL", "gemini-3.5-flash")
OPENAI_MINI_MODEL = os.getenv("LAB_MINI_MODEL", "gemini-2.5-flash")
...
def call_openai(
    ...
    """
    Gọi Chat Completions API (Gemini qua endpoint tương thích OpenAI),
    trả về nội dung phản hồi + độ trễ.

    Args:
        prompt:      Tin nhắn của người dùng.
        model:       Model sử dụng (mặc định: gemini-3.5-flash).
    ...
    """
    # TODO: import OpenAI, tạo client (api_key + base_url), gọi
...
def call_openai_mini(
    ...
    """
    Gọi API với model gemini-2.5-flash — nhanh hơn và rẻ hơn.
...
# ---------------------------------------------------------------------------
# Task 1.3 — So sánh Gemini 3.5 Flash vs Gemini 2.5 Flash
# ---------------------------------------------------------------------------
def compare_models(prompt: str) -> dict:
    ...
    Returns:
        Dict với các key (tên key giữ tiền tố gpt4o_ cho khớp bộ test — hiểu là
        "model chính" = Gemini 3.5 Flash, còn mini_ = Gemini 2.5 Flash):
            - "gpt4o_response":      str
            - "mini_response":       str
            - "gpt4o_latency":       float
            - "mini_latency":        float
    ...
    Gợi ý:
        pricing = PRICING_PER_1K_TOKENS.get(
            OPENAI_MODEL, PRICING_PER_1K_TOKENS["gemini-3.5-flash"]
        )
    ...
    gpt4o_cost_estimate = (len(gpt4o_response.split()) / 0.75) / 1000 * pricing["output"]
    return {
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    Gợi ý:
        ...
        tiktoken là bộ đếm của OpenAI nên KHÔNG biết tên model Gemini, và
        lần chạy đầu còn cần tải bộ mã hóa từ mạng. Hãy bọc trong try/except
        — khi lỗi (model Gemini, model lạ, offline) thì dùng ước lượng dự
        phòng: max(1, len(text) // 4)   (trung bình 1 token ≈ 4 ký tự).
        Với Gemini, con số này chỉ là xấp xỉ — đủ dùng cho mục đích học.
    """
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    Gợi ý:
        pricing = PRICING_PER_1K_TOKENS.get(
            model, PRICING_PER_1K_TOKENS["gemini-3.5-flash"]
        )
        input_cost = input_tokens / 1000 * pricing["input"]
        (.get với fallback: model không có trong bảng giá — ví dụ model NIM
         miễn phí — thì lấy giá gemini-3.5-flash làm tham chiếu học tập)
    """
...
def streaming_chatbot() -> None:
    ...
    Gợi ý:
        - Tạo client như Task 1.1 (api_key=GEMINI_API_KEY, base_url=GEMINI_BASE_URL).
    ...
    while True: 
        ...
        print()  # New line after the assistant's response
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": assistant_response})
        history = history[-6:]  # Keep only the last 3 turns (6 messages)
...
def run_assistant(
    ...
    while True:
        ...
        num_turns += 1
        total_tokens += count_tokens(user_msg) + count_tokens(assistant_response)
        cost_info = estimate_cost(user_msg, assistant_response)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    Cột: Prompt | Gemini 3.5 Flash | Gemini 2.5 Flash | Latency 3.5 | Latency 2.5
    Gợi ý: cắt text dài còn 40 ký tự cho dễ nhìn.
    """
    # TODO (bonus): dựng chuỗi bảng và trả về
    table = "Prompt | Gemini 3.5 Flash | Gemini 2.5 Flash | Latency 3.5 | Latency 2.5\n"
    table += "-" * 100 + "\n"
    for result in results:
        prompt = (result["prompt"][:37] + "...") if len(result["prompt"]) > 40 else result["prompt"]
        gpt4o_response = (result["gpt4o_response"][:37] + "...") if len(result["gpt4o_response"]) > 40 else result["gpt4o_response"]
        mini_response = (result["mini_response"][:37] + "...") if len(result["mini_response"]) > 40 else result["mini_response"]
        latency_3_5 = f"{result['gpt4o_latency']:.2f}"
        latency_2_5 = f"{result['mini_latency']:.2f}"
        table += f"{prompt} | {gpt4o_response} | {mini_response} | {latency_3_5} | {latency_2_5}\n"
    return table
...
if __name__ == "__main__":
    print("=== So sánh model (Gemini 3.5 Flash vs Gemini 2.5 Flash) ===")
